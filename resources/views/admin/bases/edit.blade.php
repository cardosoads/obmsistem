        ...
        <div class="flex justify-between items-center">
            <div class="flex items-center space-x-4">
                <div class="w-16 h-16 rounded-xl flex items-center justify-center" style="background: rgba(248, 171, 20, 0.2);">
                    <i class="fas fa-edit text-3xl" style="color: #F8AB14;"></i>
                </div>
                <div>
                    <h1 class="text-3xl font-bold text-white mb-2">Editar Base</h1>
                    <p class="text-blue-100">Atualize as informações da base: <strong>{{ $base->name }} ({{ $base->uf }})</strong></p>
                </div>
            </div>
...
<nav class="flex mb-6" aria-label="Breadcrumb">
    <ol class="inline-flex items-center space-x-1 md:space-x-3">
        ...
        <li aria-current="page">
            <div class="flex items-center">
                <i class="fas fa-chevron-right text-gray-400 mx-2"></i>
                <span class="text-sm font-medium text-gray-500">Editar: {{ $base->name }}</span>
            </div>
        </li>
    </ol>
</nav>
...
<div class="bg-white rounded-xl shadow-lg overflow-hidden">
    ...
    <form action="{{ route('admin.bases.update', $base->id) }}" method="POST" class="p-8">
        @csrf
        @method('PUT')
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function carregarEstados() {
        try {
            ...
            if (data.success) {
                ...
                // Se há uma UF atual, carregar suas cidades
                if (baseAtual.uf) {
                    await carregarMunicipiosEdicao(baseAtual.uf);
                }
            } else {
    ...
    async function carregarMunicipiosEdicao(uf) {
        ...
        try {
            ...
            if (data.success) {
                ...
                // Selecionar a cidade atual
                if (baseAtual.name) {
                    citySelect.value = baseAtual.name;
                }
                
                // Atualizar regional
                const selectedUfOption = document.querySelector(`#uf option[value="${uf}"]`);
                if (selectedUfOption) {
                    regionalInput.value = selectedUfOption.dataset.regiao || baseAtual.regional;
                }
            } else {
    ...
    // Carregar municípios quando UF for alterada
