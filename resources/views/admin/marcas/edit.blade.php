        ...
        <div class="flex justify-between items-center">
            <div class="flex items-center space-x-4">
                <div class="w-12 h-12 rounded-xl flex items-center justify-center" style="background: rgba(248, 171, 20, 0.2);">
                    <i class="fas fa-edit text-2xl" style="color: #F8AB14;"></i>
                </div>
                <div>
                    <h1 class="text-2xl font-bold text-white">Editar Marca</h1>
                    <p class="text-blue-100">{{ $marca->name }} - {{ $marca->mercado }}</p>
                </div>
            </div>
...
<nav class="mb-6">
    <ol class="flex items-center space-x-2 text-sm">
        ...
        <li>
            <div class="flex items-center">
                <i class="fas fa-chevron-right mx-2 text-gray-400"></i>
                <a href="{{ route('admin.marcas.show', $marca->id) }}" class="text-sm font-medium hover:text-blue-600" style="color: #1E3951;">{{ $marca->name }}</a>
            </div>
        </li>
        <li>
            <div class="flex items-center">
...
<div class="bg-white rounded-xl shadow-lg overflow-hidden">
    ...
    <form action="{{ route('admin.marcas.update', $marca->id) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
